...
>[!note] 定义：$\mathfrak{A}$-指派
>对于任意的 $L$-结构 $\mathfrak{A}$ ，一个 $\mathfrak{A}$-**指派**就是指一个函数 $\alpha: \mathrm{Vb}\to A$ .

为什么要这么定义指派？这是由于在结构当中，只有变元所代表的东西是不确定的，例如在一阶算数语言 $L_\Omega$ 当中，$\dot{\mathrm{S}}0$ 是确定的，但是
...
在给定了 $\alpha$ 时，对项的指派写为 $t^{\mathfrak{A}}[\alpha]$ ，表示结构 $\mathfrak{A}$ 中的项 $t$ 在 $\mathfrak{A}$-指派 $\alpha$ 下的取值.
